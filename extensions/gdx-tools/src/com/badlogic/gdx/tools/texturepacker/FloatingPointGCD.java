 ...
 * <p>
 * Based on algorithm described <a href="http://stackoverflow.com/a/479028/506796">here</a>.
 * @author Darren Keese */
public class FloatingPointGCD {
	...
	public static float findFloatingPointGCD (Float[] input, float maxError) {
		...
		if (inputSet.size() <= 1) return Math.abs(input[0]);
		...
		for (Float f : scaledInput) {
			...
			for (Rational rat : tree) {
				...
				if (error <= maxError) valids.add(rat);
			}
	...
	private static void addSternBrocotBranch (Set<Rational> tree, Rational left, Rational right, int maxNumerator,
		int maxDenominator) {
		Rational mediant = Rational.mediant(left, right);
		if (mediant.num > maxNumerator || mediant.den > maxDenominator) return;
		tree.add(mediant);
