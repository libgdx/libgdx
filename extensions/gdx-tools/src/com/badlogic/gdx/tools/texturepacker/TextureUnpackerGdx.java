...
public class TextureUnpackerGdx {
	...
	public void splitAtlas (TextureAtlasData atlas, String outputDir) {
		...
		for (Page page : atlas.getPages()) {
			...
			if (!file.exists()) throw new RuntimeException("Unable to find atlas image: " + file.file().getAbsolutePath());
						...
						try {
							...
							if (region.findValue("split") == null) {
								...
								if (region.width != region.originalWidth || region.height != region.originalHeight) {
									...
									originalImg.drawPixmap(splitImage, (int)region.offsetX,
							...
							if (!imgDir.exists()) {
								if (!quiet) System.out.println(String.format("Creating directory: %s", imgDir.path()));
								imgDir.mkdirs();
							}
