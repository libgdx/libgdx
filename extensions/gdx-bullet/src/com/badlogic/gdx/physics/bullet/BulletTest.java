...
public class BulletTest extends GdxTest {
	...
	public void create () {
		...
		groundMesh.setVertices(new float[] { 
			20f, 0f, 20f, Color.toFloatBits(128, 128, 128, 255),
			20f, 0f, -20f, Color.toFloatBits(128, 128, 128, 255),
			-20f, 0f, 20f, Color.toFloatBits(128, 128, 128, 255),
			-20f, 0f, -20f, Color.toFloatBits(128, 128, 128, 255)
			});
		...
		for (int x = 0; x < BOXCOUNT_X; x++) {
			for (int y = 0; y < BOXCOUNT_Y; y++) {
				for (int z = 0; z < BOXCOUNT_Z; z++) {
					body = new BulletBody(boxMesh, boxInfo, BOXOFFSET_X + (float)x, BOXOFFSET_Y + (float)y, BOXOFFSET_Z + (float)z); 
					bodies.add(body);
					dynamicsWorld.addRigidBody(body.body);
				}
			}
	...
	public static void main(String[] args) {
		...
		cfg.title = test.getClass().getSimpleName();
		cfg.useGL20 = test.needsGL20();
		cfg.width = 800;
		cfg.height = 480;
		cfg.samples = 8;
