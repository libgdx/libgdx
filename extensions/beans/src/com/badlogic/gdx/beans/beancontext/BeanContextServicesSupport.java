...
/** This support class implements <code>BeanContextServices</code> interface. This class can be used directly, or be a super class
 * of your class, or be a delegate of your implementation that needs to support <code>BeanContextServices</code> interface. */
public class BeanContextServicesSupport extends BeanContextSupport implements BeanContextServices, Serializable {
	...
	/** Every child of context is companied with a <code>BCSSChild</code> instance. It can hold implementation specific information
	 * about each child.
	 * <p>
	...
	/** This class implements the <code>BeanContextServiceProvider</code> interface by wrapping a <code>BeanContextServices</code>.
	 * All services registered in the <code>BeanContextServices</code> are accessible via this wrapper service provider.
	 * <p>
	 * This class is used by <code>BeanContextServicesSupport</code> to access services provided by its parent context (if there is
	 * one).
	 * </p> */
	protected class BCSSProxyServiceProvider implements BeanContextServiceProvider, BeanContextServiceRevokedListener {
		...
		Object getService (BeanContextServices bcs, Object requestor, Class serviceClass, Object serviceSelector,
			BeanContextServiceRevokedListener listener) throws TooManyListenersException {
			return backBCS.getService(BeanContextServicesSupport.this.getBeanContextServicesPeer(), requestor, serviceClass,
				serviceSelector, new ServiceRevokedListenerDelegator(listener));
		}
	...
	private class ServiceRevokedListenerDelegator implements BeanContextServiceRevokedListener {
		...
		public void serviceRevoked (BeanContextServiceRevokedEvent event) {
			backListener.serviceRevoked(new BeanContextServiceRevokedEvent(BeanContextServicesSupport.this
				.getBeanContextServicesPeer(), event.getServiceClass(), event.isCurrentServiceInvalidNow()));
		}

	}

	/** Every servie registered in this context is companied with a <code>BCSSServiceProvider</code> instance. It can hold
	 * implementation specific information about each registered service.
	 * <p>
	...
	/** A map of all registered services - key is service class, value is <code>BCSSServiceProvider</code> object. All access to
	 * this object should be synchronized on itself. */
	 ...
	 * 
	 * @see
	 * com.esotericsoftware.android.beans.beancontext.BeanContextServices#addBeanContextServicesListener(com.esotericsoftware.android
	 * .beans.beancontext.BeanContextServicesListener)
	 */
	...
	/** Add a service to this context.
	 * <p>
	 * If the service already exists in the context, simply return false. Otherwise, the service is added and event is fired if
	 * required.
	 * </p>
	 * 
	 ...
	 * <p>
	 * First a <code>int</code> is read, indicating the number of services to read. Then pairs of service class and service
	 * provider are read one by one.
	 * </p>
	 * 
	 ...
	 * <p>
	 * First a <code>int</code> is writtern, indicating the number of serializable services. Then pairs of service class and
	 * service provider are writtern one by one.
	 * </p>
	 * 
	 ...
	 * 
	 * @see com.badlogic.gdx.beans.beancontext.BeanContextSupport#createBCSChild(java.lang.Object, java.lang.Object) */
	...
	public Iterator getCurrentServiceSelectors (Class serviceClass) {
		...
		return provider == null ? null : new BCSIterator(provider.getCurrentServiceSelectors(getBeanContextServicesPeer(),
			serviceClass));
	}
	...
	/** Get a service instance on behalf of the specified child of this context, by calling the registered service provider, or by
	...
	/** Called after the parent context is updated. The implementation checks if the parent context is a
	 * <code>BeanContextServices</code>. If it is, then a <code>BCSSProxyServiceProvider</code> is created to delegate service
	 * requests to the parent context.
	 * 
	...
	/** Called before the parent context is updated. The implementation releases any service that is currently provided by the
	 * parent context.
	 * 
	...
	private void releaseServiceWithoutCheck (BeanContextChild child, BCSSChild bcssChild, Object requestor, Object service,
			...
			for (Iterator iter = bcssChild.serviceRecords.iterator(); iter.hasNext();) {
				...
				if (rec.requestor == requestor && rec.service == service) {
					...
					if (callRevokedListener && rec.revokedListener != null) {
						rec.revokedListener.serviceRevoked(new BeanContextServiceRevokedEvent(getBeanContextServicesPeer(),
	 ...
	 * 
	 * @see
	 * com.esotericsoftware.android.beans.beancontext.BeanContextServices#removeBeanContextServicesListener(com.esotericsoftware
	 * .android.beans.beancontext.BeanContextServicesListener)
	 */
	 ...
	 * <p>
	 * The given service provider is unregistered and a <code>BeanContextServiceRevokedEvent</code> is fired. All registered
	...
	private void notifyServiceRevokedToServiceUsers (Class serviceClass, BeanContextServiceProvider serviceProvider,
	...
	private void notifyServiceRevokedToServiceUsers (Class serviceClass, BeanContextServiceProvider serviceProvider,
		...
		synchronized (bcssChild.child) {
			for (Iterator it = bcssChild.serviceRecords.iterator(); it.hasNext();) {
				...
				if (rec.serviceClass == serviceClass && rec.provider == serviceProvider && rec.revokedListener != null
					&& !rec.isDelegate) {
					rec.revokedListener.serviceRevoked(new BeanContextServiceRevokedEvent(getBeanContextServicesPeer(), serviceClass,
