...
/** The <code>Introspector</code> is a utility for developers to figure out which properties, events, and methods a JavaBean
 * supports.
 * <p>
 * The <code>Introspector</code> class walks over the class/superclass chain of the target bean class. At each level it checks if
 * there is a matching <code>BeanInfo</code> class which provides explicit information about the bean, and if so uses that
 * explicit information. Otherwise it uses the low level reflection APIs to study the target class and uses design patterns to
 * analyze its behaviour and then proceeds to continue the introspection with its baseclass.
 * </p>
 * <p>
 * To look for the explicit information of a bean:
 * </p>
 * <ol>
 * <li>The <code>Introspector</code> appends "BeanInfo" to the qualified name of the bean class, try to use the new class as the
 * "BeanInfo" class. If the "BeanInfo" class exsits and returns non-null value when queried for explicit information, use the
 * explicit information</li>
 * <li>If the first step fails, the <code>Introspector</code> will extract a simple class name of the bean class by removing the
 * package name from the qualified name of the bean class, append "BeanInfo" to it. And look for the simple class name in the
 * packages defined in the "BeanInfo" search path (The default "BeanInfo" search path is <code>sun.beans.infos</code>). If it
 * finds a "BeanInfo" class and the "BeanInfo" class returns non-null value when queried for explicit information, use the
 * explicit information</li>
 * </ol> */
...
public class Introspector extends java.lang.Object {
	...
	/** Constant values to indicate that the <code>Introspector</code> will ignore the <code>BeanInfo</code> class of the current
	 * bean class. */
	public static final int IGNORE_IMMEDIATE_BEANINFO = 2;

	/** Constant values to indicate that the <code>Introspector</code> will use all <code>BeanInfo</code> class which have been
	 * found. This is the default one. */
	...
	private static Map<Class<?>, StandardBeanInfo> theCache = Collections
		.synchronizedMap(new WeakHashMap<Class<?>, StandardBeanInfo>(DEFAULT_CAPACITY));
	...
	/** Gets the <code>BeanInfo</code> object which contains the information of the properties, events and methods of the specified
	 * bean class.
	 * 
	 * <p>
	 * The <code>Introspector</code> will cache the <code>BeanInfo</code> object. Subsequent calls to this method will be answered
	...
	/** Gets the <code>BeanInfo</code> object which contains the information of the properties, events and methods of the specified
	 * bean class. It will not introspect the "stopclass" and its super class.
	 * 
	 * <p>
	 * The <code>Introspector</code> will cache the <code>BeanInfo</code> object. Subsequent calls to this method will be answered
	 * with the cached data.
	 ...
	 * @param stopClass the sopt class which should be super class of the bean class. May be null.
	 * @return the <code>BeanInfo</code> of the bean class.
	 * @throws IntrospectionException */
	 ...
	 * <li>If <code>flag==IGNORE_IMMEDIATE_BEANINFO</code>, the <code>Introspector</code> will ignore the <code>BeanInfo</code>
	 * class of the current bean class.</li>
	 * <li>If <code>flag==USE_ALL_BEANINFO</code>, the <code>Introspector</code> will use all <code>BeanInfo</code> class which
	 * have been found.</li>
	 * </ol>
	 * <p>
	 * The <code>Introspector</code> will cache the <code>BeanInfo</code> object. Subsequent calls to this method will be answered
	 ...
	 * 
	 * @param beanClass the specified bean class.
	 * @param flags the flag to control the usage of the explicit <code>BeanInfo</code> class.
	 * @return the <code>BeanInfo</code> of the bean class.
	 * @throws IntrospectionException */
	...
	private static StandardBeanInfo getBeanInfoImpl (Class<?> beanClass, Class<?> stopClass, int flags)
		...
		if (beanSuperClass != stopClass) {
			if (beanSuperClass == null) throw new IntrospectionException("Stop class is not super class of bean class"); //$NON-NLS-1$
	 ...
	 * @param classLoader
	 * 
	 * @return A BeanInfo object which is an instance of the Class named theBeanInfoClassName null if the Class does not exist or
	 * if there are problems instantiating the instance
	 */
