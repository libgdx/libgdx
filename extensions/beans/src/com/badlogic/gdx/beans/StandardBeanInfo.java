...
import static com.badlogic.gdx.beans.Introspector.*;
...
class StandardBeanInfo extends SimpleBeanInfo {
	...
	StandardBeanInfo (Class<?> beanClass, BeanInfo explicitBeanInfo, Class<?> stopClass) throws IntrospectionException {
		...
		if (explicitBeanInfo != null) {
			...
			if (events != null) explicitEvents = true;
			if (methods != null) explicitMethods = true;
			if (properties != null) explicitProperties = true;
		}
		...
		for (int i = 0; i < superDescs.length; i++) {
			...
			if (superIndexedType == null) {
				...
				if (subIndexedType == null) {
					// Same property type
					if (subType != null && superType != null && subType.getName() != null
					...
					if (superType != null && (superType.isArray())
						&& (superType.getComponentType().getName().equals(subIndexedType.getName()))) {
				...
				if (subIndexedType == null) { // Sub is PropertyDescriptor
					if (subType != null && subType.isArray()
						&& (subType.getComponentType().getName().equals(superIndexedType.getName()))) {
						// Same type
						if (subGet != null) {
	...
	private MethodDescriptor[] introspectMethods (boolean includeSuper, Class<?> introspectorClass) {
		...
		if (basicMethods == null || basicMethods.length == 0) return null;
	 ...
	 * 
	 * @param stopClass - the to introspecting at
	...
	private EventSetDescriptor[] introspectEvents () throws IntrospectionException {
		...
		for (Map.Entry<String, HashMap> entry : eventTable.entrySet()) {
			...
			EventSetDescriptor eventSetDescriptor = new EventSetDescriptor(decapitalize(entry.getKey()), listenerType,
