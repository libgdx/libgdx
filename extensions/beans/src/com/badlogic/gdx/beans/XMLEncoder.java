...
/** <code>XMLEncoder</code> extends <code>Encoder</code> to write out the encoded statements and expressions in XML format. The XML
 * can be read by <code>XMLDecoder</code> later to restore objects and their states.
 * <p>
...
public class XMLEncoder extends Encoder {
	...
	/*
	 * if any expression or statement references owner, it is set true in method recordStatement() or recordExpressions(), and, at
	 * the first time flushObject() meets an owner object, it calls the flushOwner() method and then set needOwner to false, so
	 * that all succeeding flushing of owner will call flushExpression() or flushStatement() normally, which will get a reference
	 ...
	 * 
	 * @param out the output stream where XML is written to */
	 ...
	 * <p>
	 * The implementation write the XML header first if it has not been written. Then all pending objects since last flush are
	 * written.
	 * </p> */
	...
	private void flushExpression (Object obj, Record rec, int indent, boolean asStatement) {
		// flush
		Statement stat = asStatement ? new Statement(rec.exp.getTarget(), rec.exp.getMethodName(), rec.exp.getArguments())
			: rec.exp;
	...
	private boolean isBasicType (Object value) {
		return value == null || value instanceof Boolean || value instanceof Byte || value instanceof Character
			|| value instanceof Class<?> || value instanceof Double || value instanceof Float || value instanceof Integer
			|| value instanceof Long || value instanceof Short || value instanceof String || value instanceof Proxy;
	}
	...
	/** Imperfect attempt to detect a dead loop. This works with specific patterns that can be found in our AWT implementation. See
	 * HARMONY-5707 for details.
	 * 
