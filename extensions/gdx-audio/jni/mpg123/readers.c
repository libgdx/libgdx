...
static ssize_t timeout_read(mpg123_handle *fr, void *buf, size_t count) {
	return -1;
	// FIXME mzechner: had to get rid of this call, not used in our android projects
	/* struct timeval tv;
	ssize_t ret = 0;
	fd_set fds;
	...
	ret = -1; // FIXME mzechner, this is stupid :D select(fr->rdat.filept+1, &fds, NULL, NULL, &tv);
...
static int default_init(mpg123_handle *fr) {
	// FIXME mzechner: got rid of this due to *nix syscalls
	/*if(fr->p.timeout > 0)	{
		int flags;
