 ...
 * through the method {@code setUniqueness}. Uniqueness is disabled by default.
 * 
 * <p>
 * The <em>head</em> of this queue is the <em>least</em> element with respect to
 * the specified ordering. If multiple elements are tied for least value
 * (provided that uniqueness is set to false), the head is one of those elements
 * -- ties are broken arbitrarily. The queue retrieval operations {@code poll},
 * {@code remove}, {@code peek}, and {@code element} access the element at the
 * head of the queue.
 * 
 * <p>
 ...
 * 
 * <p>
 * Iterating the queue with the method {@link #get() get(index)} is <em>not</em>
 ...
 * enqueing and dequeing methods ({@code add} and {@code poll} ; and constant
...
public class PriorityQueue<E> {
	 ...
	 * ordered by the elements' natural ordering: For each node n in the heap
	...
	/**
	 * A flag indicating whether elements inserted into the queue must be
	 * unique.
	 */
	...
	/**
	 * Returns a value indicating whether only unique elements are allowed to be
	 * inserted.
	 */
	...
	/**
	 * Sets a flag indicating whether only unique elements are allowed to be
	 * inserted.
	 */
	...
	/**
	 * Retrieves, but does not remove, the head of this queue. If this queue is
	...
	/**
	 * Retrieves the element at the specified index. If such an element doesn't
	 * exist {@code null} is returned.
	 * <p>
	 * Iterating the queue by index is <em>not</em> guaranteed to traverse the
