    ...
    switch (channels) {
      case 1:
        switch (bitDepth) {
          ...
          default: throw new GdxRuntimeException("Sound: Bit depth must be 8, 16, 32 or 64.");
        }
        break;
      ...
      default:
        throw new GdxRuntimeException("Sound: Invalid number of channels. " +
          "Must be mono, stereo, quad, 5.1, 6.1 or 7.1.");
    }
    if (channels >= 4) {
      if (bitDepth == 8) format--; // Use 8-bit AL_FORMAT instead
      else if (bitDepth == 32) format++; // Use 32-bit instead
      else if (bitDepth != 16)
        throw new GdxRuntimeException("Sound: Bit depth must be 8, 16 or 32 when 4+ channels are present.");
    }
    return format;
  }
