 ...
 * Either use this and pass an activity to it, or create an {@link Activity} that derives from {@link AndroidApplication}. In
 ...
 * 
 * @author mzechner */
public class AndroidApplicationBackend implements Application {
	static {
	 ...
	 * input, render via OpenGL and so on. If useGL20IfAvailable is set the AndroidApplicationBackend will try to create an OpenGL ES 2.0
	...
	public void initialize (ApplicationListener listener, AndroidApplicationConfiguration config) {
		...
		input = AndroidInputFactory.newAndroidInput(this, activity, graphics.view, config);
		audio = new AndroidAudio(activity, config);
		files = new AndroidFiles(activity.getAssets(), activity.getFilesDir().getAbsolutePath());
		...
		try {
			...
			log("AndroidApplicationBackend", "Content already displayed, cannot request FEATURE_NO_TITLE", ex);
		}
	...
	protected void hideStatusBar (AndroidApplicationConfiguration config) {
		...
		try {
			...
			log("AndroidApplicationBackend", "Can't hide status bar", e);
		}
	}
