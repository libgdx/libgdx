...
final class AndroidFont extends Font {
	...
	private int getFontStyle (FontStyle style) {
		if (style == FontStyle.Bold) return Typeface.BOLD;
		if (style == FontStyle.BoldItalic) return Typeface.BOLD_ITALIC;
		if (style == FontStyle.Italic) return Typeface.ITALIC;
		if (style == FontStyle.Plain) return Typeface.NORMAL;
	 ...
	 * {@inheritDoc}
	 */
	@Override public int getGlyphAdvance (char character) {
	 ...
	 * {@inheritDoc}
	 */
	@Override public Pixmap getGlyphBitmap (char character) {
		Rect rect = new Rect();
		paint.getTextBounds("" + character, 0, 1, rect);
		Bitmap bitmap = Bitmap.createBitmap(rect.width() == 0 ? 1 : rect.width() + 5, getLineHeight(),
			AndroidPixmap.getInternalFormat(Font.pixelFormat));
	 ...
	 * {@inheritDoc}
	 */
	@Override public int getLineGap () {
	 ...
	 * {@inheritDoc}
	 */
	@Override public void getGlyphBounds (char character, Rectangle rect) {
