...
public final class JoglGraphics implements Graphics, RenderListener {
	...
	JoglGraphics (final JoglApplication application, String title, int width, int height, boolean useGL2IfAvailable) {
		...
		frame.addWindowListener(new WindowAdapter() {
			public void windowClosing (WindowEvent e) {
				if (application.listener != null) application.listener.pause();
				graphicPanel.dispose();
				if (application.listener != null) application.listener.destroy();
			}
		});
	...
	@Override public void setRenderListener (RenderListener listener) {
		if (this.listener != null) graphicPanel.removeGraphicListener(this.listener);
