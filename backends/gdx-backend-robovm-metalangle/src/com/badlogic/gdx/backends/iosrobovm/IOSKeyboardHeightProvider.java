...
/** DO NOT EDIT THIS FILE - it is machine generated Pushes keyboard show/hide events to an {@link IOSKeyboardObserver}. Owns the
 * notification registration, the notification parsing and the keyboard-matched animation: the observer is invoked one frame after
 * the notification (via postRunnable) inside a UIView animation block using the keyboard's own duration and curve, so any UIView
 * property writes the observer performs animate alongside the keyboard. */
