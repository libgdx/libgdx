...
/**
 * An InputListener is used to receive input events from a {@link Application}. These
 * events might arrive asynchronously in a UI thread, the {@link Application} will buffer those events
 * and invoke the methods of the listene in the rendering thread to get rid of the need to synchronize
 * the UI and renderer thread for input. If an input listener signals that it processed an event by 
 * returning true the event will not be passed to other listeners in a listener chain. Note that
