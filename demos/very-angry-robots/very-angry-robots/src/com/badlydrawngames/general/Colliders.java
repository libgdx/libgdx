...
public final class Colliders {
	...
	public static <V extends GameObject, W extends GameObject, T extends V, U extends W> void collide(T a, U b, ColliderHandler<V, W> callback) {
		if (a != b) {
			if (a.intersects(b)) {
	...
	public static <V extends GameObject, W extends GameObject, T extends V, U extends W> void collide(T go, Array<U> gos, ColliderHandler<V, W> callback) {
	...
	public static <V extends GameObject, W extends GameObject, T extends V, U extends W> void collide(Array<T> a, Array<U> b, ColliderHandler<V, W> callback) {
		if (a != b) {
			for (int i = a.size - 1; i >= 0; i--) {
				T go = a.get(i);
	...
	public static <U extends GameObject, T extends U> void collide(Array<T> gos, Array<Rectangle> rects, SceneryHandler<U> callback) {
		for (int i = gos.size - 1; i >= 0; i--) {
			T go = gos.get(i);
	...
	/**
	 * Removes game objects that are marked as in collision, calling a removal handler for each one that is marked.
	 * Note that the "U extends T" part of this method's signature means, for example, that PlayerShot and RobotShot
	 ...
	 */
	public static <T extends GameObject, U extends T> void removeMarkedCollisions(Pool<U> pool, Array<U> gos, RemovalHandler<T> handler) {
