...
/**
 * <p>A simple image packer class based on the nice algorithm by blackpawn.</p>
 * 
 * <p>See http://www.blackpawn.com/texts/lightmaps/default.html for details.</p>
 * 
 * <p><b>Usage:</b> instanciate an <code>ImagePacker</code> instance, load and optionally sort
 * the images you want to add by size (e.g. area) then insert each image via a call
 * to {@link #insertImage(String, BufferedImage)}. When you are done with inserting images
 * you can call {@link #getImage()} for the {@link BufferedImage} that holds the packed images.
 * Additionally you can get a <code>Map<String, Rectangle></code> where the keys the names you specified
 * when inserting and the values are the rectangles within the packed image where that specific
 * image is located. All things are given in pixels.</p>
 * 
 * <p>See the {@link #main(String[])} method for an example that will generate 100 random images, pack them and then
 * output the packed image as a png along with a json file holding the image descriptors.</p>
 * 
 * <p>In some cases it is beneficial to add padding and to duplicate the border pixels of an inserted image so
 * that there is no bleeding of neighbouring pixels when using the packed image as a texture. You can specify
 * the padding as well as whether to duplicate the border pixels in the constructor.</p>
 * 
 * <p>Happy packing!</p>
 * 
 * @author mzechner
...
public class ImagePacker 
{
	...
	/**
	 * <p>Creates a new ImagePacker which will insert all supplied images into a
	 * <code>width</code> by <code>height</code> image. <code>padding</code> specifies
	 * the minimum number of pixels to insert between images. <code>border</code> will
	 * duplicate the border pixels of the inserted images to avoid seams when rendering
	 * with bi-linear filtering on.</p> 
	 * 
	...
	/**
	 * <p>Inserts the given image. You can later on retrieve the images position
	 * in the output image via the supplied name and the method {@link #getRects()}.</p>
	 * 
