...
public class JsonMatcherTests {
	...
	public void singlePatterns () {
		...
		test( // Whitespace in pattern (trimmed, cannot match on whitespace in field names)
			"{\"field\":{\"nested\":value}}", //
			"  field \n [ \t nested \t]", //
			"{nested=value}");
		...
		test( // Field name appears at multiple depths - only first should match
		...
		test( // Capture 2 values with same field name
			"{first:{x:1,ignored:true,second:{x:2,important:true}}}", //
			"first[x]/second[x]", //
			"{x=2}");
