...
/**
 * <p>A skin defines graphical resources like {@link NinePatch}, {@link TextureRegion}, {@link Color} and {@link BitmapFont}
 * instances as well as widget styles.</p> 
 * 
 * <p>Every widget found in this UI API, like {@link Button} or {@link SplitPane} has
 * an associated style, like {@link ButtonStyle} or {@link SplitPaneStyle}. These styles are defined in an XML
 * file and reference (nine-patch)regions in a single texture, colors and fonts to be used for any widget
 * instantiated with that style.</p>
 * 
 ...
 * method.</p>
 * 
 * <p>A skin XML file has a root element called &ltskin&gt. Within this tag there are two elements excepted:</p>
 * 
 * <pre>
 * {@code
 * <skin> 
 ...
 * </pre>
 * 
 * <p>The {@code <library>} element contains the definitions of nine-patches, regions, colors and fonts.
 * Every one of these has to have a unique name. When defining a widget style in the {@code <widgetstyles>} section
 * you will reference these.</p>
 * 
 * <p>Regions, which are loaded as {@link TextureRegion} instances, define an area in the skin's texture. They
 * are specified in pixels, with the origin being in the top left corner of the texture image, with the y-axis
 * pointing downwards. Coordinates and sizes are given as integers relative to that coordinate system.</p>
 * 
 * <pre>
 * {@code 
 ...
 * 
 * 
 * <p>Nine-patches are composed of 9 regions and are defined like this:</p>
 * 
 * <pre>
 * {@code
 ...
 * </pre>
 * 
 * <p>Note that the regions in each column have to have the same width, and the regions in each
 * row have to have the same width!</pre>
 * 
 * <p>Colors are defined as RGBA values in the range [0,1] like this:</p>
 * <pre>
 * {@code 
 ...
 * </pre>
 * 
 * <p>Fonts are defined like this:</p>
 * <pre>
 * {@code 
 ...
 * </pre>
 * 
 * <p>The font's files have to be internal files {@link Files#internal(String)}, the path given is relative to the
 * internal files path (e.g. assets/ on Android, the apps root directory on the desktop).</p>
 * 
 * <p>These four types of graphical resources can then be referenced in widget style definitions via their name. A
 * button style for example needs a ninepatch for its background in the "down" (pressed) state, a ninepatch for its background
 * in the "up" (unpressed) state and a font and font color to display its label text. A button style could be defined like this:</p>
 * <pre>
 * {@code
 ...
 * </pre> 
 * 
 * <p>The {@code <buttonstyle>} would be a child of the {@code <widgetstyles>} element. Its attributes reference nine-patches, fonts
 * and colors from the {@code <librar>} element via their names. The above example requires a nine-patch named "button-down-patch" to be
 * defined for example.</p> 
 * 
 * <p>Refer to the invidual widget classes like {@link Button}, {@link ComboBox}, {@link SplitPane} and so on to find out
 * how to define a style for them in the XML file.</p>
 * @author mzechner
 *
 */
public class Skin implements Disposable {
	...
	private void parseColors(Element library) {
		for(Element color: library.getChildrenByName("color")) {
			Color col = new Color(color.getFloatAttribute("r"),
								  color.getFloatAttribute("g"), 
								  color.getFloatAttribute("b"), 
	...
	private TextureRegion parseRegion(Element region) {
		return new TextureRegion(texture, 
								 region.getIntAttribute("x"), region.getIntAttribute("y"),
	...
	/**
	 * Creates a new {@link SplitPane}, using the given style. There is no
	 * 'default' version of this method as a vertical split pane is likely to
	 * use a different style than a horizontal split pane.
	 * 
	 ...
	 */
	public SplitPane newSplitPane(String name, Stage stage, Actor firstWidget, Actor secondWidget, boolean vertical, int prefWidth, int prefHeight, String style) {
