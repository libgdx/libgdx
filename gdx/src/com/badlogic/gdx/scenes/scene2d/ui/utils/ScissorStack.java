...
/**
 * A scissor stack is a stack of {@link Rectangle} to be used for clipping via {@link GLCommon#glScissor(int, int, int, int)}. When
 * a new Rectangle is pushed onto the stack, it will be merged with the current top of stack. The minimum area of overlap is then
 * set as the real top of the stack.
 * @author mzechner
 *
 */
public class ScissorStack {
	...
	/**
	 * Pushes a new scissor {@link Rectangle} onto the stack, merging it with the current
	 * top of the stack. The minimal area of overlap between the TOS rectangle and the provided
	 * rectangle is pushed onto the stack. This will invoke {@link GLCommon#glScissor(int, int, int, int)}
	 * with the final TOS rectangle. In case no scissor is yet on the stack this will also enable
	 * {@link GL10#GL_SCISSOR_TEST} automatically.
	...
	/**
	 * Pops the current scissor rectangle from the stack and sets the new scissor area to the 
	 * new TOS rectangle. In case no more rectangles are on the stack, {@link GL10#GL_SCISSOR_TEST}
	 * is disabled.
	 */
	...
	/**
	 * Calculates a scissor rectangle in OpenGL ES window coordinates from a {@link Camera}, a transformation {@link Matrix4}
	 * and an axis aligned {@link Rectangle}. The rectangle will get transformed by the camera and transform matrices and is
	 * then projected to screen coordinates. Note that only axis aligned rectangles will work with this method. If either
	 * the Camera or the Matrix4 have rotational components, the output of this method will not be suitable for {@link GLCommon#glScissor(int, int, int, int)}.
	...
	/**
	 * Transforms a point to real window coordinates (as oposed to OpenGL ES window coordinates), where
	 * the origin is in the top left and the the y-axis is pointing downwards
	 * @param camera the {@link Camera}
