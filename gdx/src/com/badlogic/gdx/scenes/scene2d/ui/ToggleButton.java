 ...
 * 
 * <h2>Layout</h2>
 * The (preferred) width and height of an ImageToggleButton are derived from the border patches in the
 * background {@link NinePatch} as well as the bounding box of the multi-line label displayed inside the
 * ToggleButton. Use {@link #setPrefSize(int, int)} to programmatically change the size
 * to your liking. In case the width and height you set are to small for the contained label you
 * will see artifacts.
 * 
 * <h2>Style</h2>
 * A ToggleButton is a {@link Widget} displaying a background {@link NinePatch} as well as
 * multi-line text with a specific font and color. The style is defined via an instance
 * of {@link ToggleButtonStyle}, which can be either done programmatically or via a {@link Skin}.</p>
 * 
 ...
 * 
 * <ul>
 * <li>The <code>name</code> attribute defines the name of the style which you can later use with {@link Skin#newToggleButton(String, String, String)}.</li>
 * <li>The <code>down</code> attribute references a {@link NinePatch} by name, to be used as the toggle button's background when it is pressed</li>
 * <li>The <code>up</code> attribute references a {@link NinePatch} by name, to be used as the toggle button's background when it is not pressed</li>
 * <li>The <code>font</code> attribute references a {@link BitmapFont} by name, to be used to render the text on the toggle button</li>
 * <li>The <code>fontColor</code> attribute references a {@link Color} by name, to be used to render the text on the toggle button</li>
 * </ul> 
 * 
 * @author mzechner
 *
...
public class ToggleButton extends Widget {
	...
	/**
	 * Creates a new ToggleButton. The width and height are determined by the background {@link NinePatch}
	 * border patches as well as the bounding box around the contained text label.
	 * @param name the name
