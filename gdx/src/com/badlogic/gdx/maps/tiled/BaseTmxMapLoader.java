...
public abstract class BaseTmxMapLoader<P extends BaseTmxMapLoader.Parameters> extends AsynchronousAssetLoader<TiledMap, P> {
    ...
    public static class Parameters extends AssetLoaderParameters<TiledMap> {
        ...
        /**
         * Whether to flip all Y coordinates so that Y positive is up. All libGDX renderers require flipped Y
         * coordinates, and thus
         * flipY set to true. This parameter is included for non-rendering related purposes of TMX files, or custom
         * renderers.
         */
     ...
     */
    protected ObjectMap<Integer, MapProperties> idToPropsMap;
    /**
    ...
    protected TiledMap loadTiledMap(FileHandle tmxFile, P parameter, ImageResolver imageResolver) {
        ...
        this.idToPropsMap = new ObjectMap<>();
        ...
        // in Tiled the final parallax scrolling factor of a layer is the multiplication of its factor with all its
        // parents
    ...
    protected void loadObject(TiledMap map, MapObjects objects, Element element, float heightInPixels) {
        if (element.getName().equals("object")) {
            ...
            float y = (flipY ? (heightInPixels - element.getFloatAttribute("y", 0)) : element.getFloatAttribute("y",
                    0)) * scaleY;
                ...
                if (type != null && type.equals("object")) {
                    ...
                    try {
                        ...
                        Runnable fetchMapProps = new Runnable() {
                            @Override
                            public void run() {
                                MapProperties props = idToPropsMap.get(_value);
                        ...
                        Gdx.app.error("BaseTmxMapLoader", "Error parsing property [" + name + "] of type \"object\"",
                                exception);
                    }
                } else {
    ...
    protected abstract void addStaticTiles(FileHandle tmxFile, ImageResolver imageResolver, TiledMapTileSet tileset,
                                           Element element, Array<Element> tileElements, String name, int firstgid,
                                           int tilewidth, int tileheight, int spacing,
                                           int margin, String source, int offsetX, int offsetY, String imageSource,
                                           int imageWidth, int imageHeight, FileHandle image);
