...
public abstract class BaseTmxMapLoader<P extends BaseTiledMapLoader.Parameters> extends BaseTiledMapLoader<P> {
	 ...
	 * .tx file. Unlike when loading every other object, we need to read the property attributes contained in the .tx file first.
		...
		if (element.getName().equals("object")) {
			...
			if (object == null) {
				...
				if (gid != null) {
					...
					float objectWidth = Float
						.parseFloat(combinedAttributes.get("width", String.valueOf(textureRegion.getRegionWidth())));
