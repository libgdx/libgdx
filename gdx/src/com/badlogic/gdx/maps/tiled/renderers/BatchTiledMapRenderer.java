		...
		if (!layer.isRepeatX() && !layer.isRepeatY()) {
			...
			for (int i = 0; i <= repeatX; i++) {
				for (int j = 0; j <= repeatY; j++) {
					 ...
					 * would tile the image layer from it's x,y/offset position, but what we want is the texture to be tiled from
					 * left to right, within the camera's bounds, while keeping its offset.
					 * Now with rx1 = (viewBounds.x-(viewBounds.x% imageBounds.width)) we can ignore the x1 starting position as it
					 * no longer matters, we want the starting position flush to camera's x position. Taking camera's x position we
					 * need to subtract that by the camera modulus of the image width to get the remainder, so we know
					 * how far to move the image to get flush with the camera. But of course we are not done yet, if the image starts
					 * off with an x/y position that is not zero, (an offset), we must now negate this. With rx1 =
					 ...
					 * starting position and 0. Finally, this needs to be tiled, so we use rx1 =
					 * ((viewBounds.x)-(viewBounds.x% imageBounds.width))+(i -2)*imageBounds.width+(x1% imageBounds.width);
					 * Then this is where our loops comes in, we use the repeatX = (viewBounds.width / imageBounds.width) +3 to
					 * find out how many textures we can fit in the camera + 3 more(so we can expand beyond the borders )
					 * so we dont have pop in and out around the edges of the camera, we use + (i -2) to start off placing them
					 * to the left of the camera, and we fill it out going right multiplying by our image width. Until we go through
					 * the entire loop. *note for rx2 position we have to add the image width, for ry2 we add height */
