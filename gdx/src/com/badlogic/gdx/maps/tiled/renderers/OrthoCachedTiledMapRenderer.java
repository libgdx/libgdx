...
public class OrthoCachedTiledMapRenderer implements TiledMapRenderer, Disposable {
	...
	public void renderTileLayer (TiledMapTileLayer layer) {
		...
		final float layerOffsetX = layer.getRenderOffsetX() * unitScale + cacheBounds.x * (layer.getParallaxX() - 1);
		// offset in tiled is y down, so we flip it
		final float layerOffsetY = -layer.getRenderOffsetY() * unitScale - cacheBounds.y * (layer.getParallaxY() - 1);
	...
	public void renderImageLayer (TiledMapImageLayer layer) {
		...
		final float x1 = x * unitScale + cacheBounds.x * (layer.getParallaxX() - 1);
		final float y1 = y * unitScale - cacheBounds.y * (layer.getParallaxY() - 1);
		final float x2 = x1 + region.getRegionWidth() * unitScale;
		final float y2 = y1 + region.getRegionHeight() * unitScale;
