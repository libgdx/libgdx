...
/** An unordered map where the keys are objects and the values are unboxed floats. This implementation uses linear probing with
...
public class ObjectFloatMap<K> implements Iterable<ObjectFloatMap.Entry<K>> {
	...
	/** Doesn't return a value, unlike other maps. You can use {@link #get(Object, float)} with a defaultValue of {@link Float#NaN}
	 * if you want to tell with certainty that a key is not present; comparing with NaN is tricky but {@link Float#isNaN(float)}
	 * makes it easy. If isNaN returns true, you can generally act like another Map had returned null in the same situation
	 * (meaning the value is unusable). This works because this class will never insert a NaN value into the map unless one is
	 * explicitly inserted, and since NaN acts so strangely in its everyday usage, virtually all code will not place NaN in a
	 * map. */
