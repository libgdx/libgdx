...
public class IntHashMap<T> {
	...
	@SuppressWarnings("unchecked") public T get (int key) {
		int index = key & mask;
		for (Entry e = table[index]; e != null; e = e.next)
	...
	@SuppressWarnings("unchecked") public T put (int key, T value) {
		int index = key & mask;
	...
	@SuppressWarnings("unchecked") public T remove (int key) {
		int index = key & mask;
	...
	class EntryIterable implements Iterable<T> {
		...
		Iterator<T> iter = new Iterator<T>() {
			@Override public boolean hasNext () {
				if (currEntry == null) {
					if (!loadNextEntry()) return false;
				} else {
					if (currEntry.next == null && loadNextEntry() == false) return false;
				}
			...
			private boolean loadNextEntry () {
				while (true) {
					currIndex++;
					if (currIndex >= table.length) return false;
