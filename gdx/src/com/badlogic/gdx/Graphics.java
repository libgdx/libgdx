 ...
 * 
 * <p>
 * {@link Font}s and {@link Texture}s can be either managed or not managed. Managed Fonts and Textures will be restored when the
 * OpenGL context is lost. An OpenGL ES context loss happens when a user pauses the Application and switches to another
 * application on Android. On the desktop there is no concept of context loss for OpenGL. If an OpenGL context is lost all the
 * textures (and Fonts are essentially just textures) and meshes uploaded to the GPU will get invalidated and have to be reloaded.
 * Creating a Font or Texture with the managed feature means that the <code>Graphics</code> backend will take care of reloading
 * these resources as soon as the Application is resumed. You don't have to do this manually in this case. The drawback: the
 * mechanism needs additional memory to store the original resources for the reloading process in memory. Use this feature only if
 * you know that you have enough memory! Future versions might introduce a disk cache for when a Texture was created from a Pixmap
 * which was created programmatically or use the reference to the original file from which a texture was created.
 * </p>
 * 
 * <p>
 * Note that you don't have to reload managed Textures and Fonts. Doing so will effectively double the memory consumption as those
 * Textures and Fonts will be in memory twice.
 * </p>
 * 
 * @author mzechner
 * 
...
public interface Graphics {
	/**
	 * Enumeration describing different types of {@link Graphics} implementations. Might be extended by say LWJGL.
	 * @author mzechner
	 * 
	 */
	 ...
	 * @throws GdxRuntimeException in case OpenGL ES 1.1 is not supported by the hardware or the Application was configured to use
	 *            OpenGL ES 2.0
	 */
	public GL11 getGL11 ();
	 ...
	 * @throws GdxRuntimeException in case OpenGL ES 2.0 is not supported by the hardware or the Application was configured to use
	 *            OpenGL ES 1.x
	 */
	public GL20 getGL20 ();
	...
	/**
	 * Creates a new {@link Pixmap} from the given InputStream which is assumed to point to a bitmap in a readable format, e.g.
	 * PNG, JPEG. The InputStream is not closed.
	 * 
	...
	/**
	 * Creates a new {@link Pixmap} from the given file which is assumed to point to a bitmap in a readable format, e.g. PNG, JPEG.
	 * 
