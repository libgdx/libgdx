...
@SuppressWarnings("UnnecessaryParentheses") public class ArrayRegionBatch implements Disposable {
	 ...
	 static {
		  if (Gdx.gl30 == null)
				throw new RuntimeException("GL 3+ is required for ArrayRegionBatch");
	 }
	 ...
	 public void draw (final TextureArrayRegion region, final float x, final float y, final float width, final float height) {
		  ...
		  writeQuad(vertices, color, textureIndex, x, y, u, v, x, fy2, u, v2, fx2, fy2, u2, v2, fx2, y, u2, v);
	 }
	 ...
	 private void writeQuad (final float[] vertices, final float color, final float textureIndex, //
	 ...
	 private static int writeVertex (int idx, final float[] vertices, final float x, final float y, final float color,
		 final float u, final float v, final float textureIndex) {
		  vertices[idx++] = x;
		  vertices[idx++] = y;
		  vertices[idx++] = color;
		  vertices[idx++] = u;
	 ...
	 private void checkBeginCalled () {
		  if (!drawing)
				throw new IllegalStateException("begin must be called before");
	 }
