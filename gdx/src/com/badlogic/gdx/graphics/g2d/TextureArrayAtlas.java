...
public class TextureArrayAtlas implements Disposable {
	 ...
	 public static class TextureArrayAtlasData {
		  public static class Page {
				...
				public Page (final FileHandle handle, final float width, final float height, final boolean useMipMaps,
					final Pixmap.Format format, final Texture.TextureFilter minFilter, final Texture.TextureFilter magFilter,
		  ...
		  public TextureArrayAtlasData (final FileHandle packFile, final FileHandle imagesDir, final boolean flip) {
				final BufferedReader reader = new BufferedReader(new InputStreamReader(packFile.read(), Charset.forName("UTF-8")),
					64);
				try {
					 ...
					 while (true) {
						  ...
						  if (line.trim().length() == 0)
								...
								pageImage = new TextureArrayAtlas.TextureArrayAtlasData.Page(file, width, height, min.isMipMap(), format,
									min, max, repeatX, repeatY);
								pages.add(pageImage);
						  } else {
...
//        textureArray.setFilter(firstPage.minFilter, firstPage.magFilter);
		  ...
		  for (final TextureArrayAtlasData.Region region : data.regions) {
				...
				final ArrayAtlasRegion atlasRegion = new ArrayAtlasRegion(textureArray, region.page.textureIndex, region.left,
					region.top, region.rotate ? height : width, region.rotate ? width : height);
	 ...
	 public Array<TextureArrayAtlas.ArrayAtlasRegion> findRegions (final String name) {
		  ...
		  for (int i = 0, n = regions.size; i < n; i++) {
				final TextureArrayAtlas.ArrayAtlasRegion region = regions.get(i);
				if (region.name.equals(name))
					 matched.add(new TextureArrayAtlas.ArrayAtlasRegion(region));
		  }
		  return matched;
	 }
	 ...
	 static final Comparator<TextureArrayAtlas.TextureArrayAtlasData.Region> indexComparator = new Comparator<TextureArrayAtlas.TextureArrayAtlasData.Region>() {
		  @Override public int compare (final TextureArrayAtlas.TextureArrayAtlasData.Region region1,
			  final TextureArrayAtlas.TextureArrayAtlasData.Region region2) {
	 ...
	 public static class ArrayAtlasRegion extends TextureArrayRegion {
		  ...
		  public ArrayAtlasRegion (final TextureArray textureArray, final int textureIndex, final int x, final int y,
		  ...
		  /**
			...
			*/
		  @Override public void flip (final boolean x, final boolean y) {
				super.flip(x, y);
				if (x)
