...
/**
 * A perspective camera, having a position, a direction an up vector, a near
 * plane, a far plane and a field of view. Use the {@link
 * PerspectiveCamera#setViewport(float, float)} method to set the viewport from which the
 * aspect ratio is derrived from, then call {@link PerspectiveCamera#update()}
 * to update all the camera matrices as well as the {@link Frustum}. The
 * combined matrix (projection, modelview) can be retrieved via a call to
 * {@link PerspectiveCamera#getCombinedMatrix()} and directly passed to OpenGL.
 * A convenience method called {@link PerspectiveCamera#setMatrices()} exists
 * for OpenGL ES 1.x that will update the OpenGL perspective and modelview. You can also get a picking ray via
 * {@link PerspectiveCamera#getPickRay(int, int)}.
 * 
...
public final class PerspectiveCamera {
	...
	/**
	 * Updates all matrices as well as the Frustum based on the last set
	 * parameters for position, direction, up vector, field of view, near and
	 * far plane and viewport.
	 */
	public void update() {
	...
	/**
	 * Sets the projection and model view matrix of OpenGL ES 1.x to this
	 * camera's projection and model view matrix. Any previously set matrices
	 * are overwritten. Upon returning from this method the matrix mode will be
	 * GL10.GL_MODELVIEW.
	 * 
	 */
	...
	public Ray getPickRay(int screenX, int screenY) {
		return frustum.calculatePickRay(viewportWidth, viewportHeight, screenX,
				viewportHeight - screenY - 1, position, direction, up);
	}
