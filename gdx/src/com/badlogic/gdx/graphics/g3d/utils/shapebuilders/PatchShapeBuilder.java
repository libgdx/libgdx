...
public class PatchShapeBuilder extends BaseShapeBuilder {
	...
	public static void build (MeshPartBuilder builder, VertexInfo corner00, VertexInfo corner10, VertexInfo corner11,
		...
		for (int u = 0; u <= divisionsU; u++) {
			...
			data.vertTmp5.set(corner00).lerp(corner10, alphaU);
			data.vertTmp6.set(corner01).lerp(corner11, alphaU);
			for (int v = 0; v <= divisionsV; v++) {
				final short idx = builder.vertex(data.vertTmp7.set(data.vertTmp5).lerp(data.vertTmp6, (float)v / (float)divisionsV));
	...
	public static void build (MeshPartBuilder builder, Vector3 corner00, Vector3 corner10, Vector3 corner11, Vector3 corner01,
		...
		final BaseShapeData data = tlData.get();
		build(builder, data.vertTmp1.set(corner00, normal, null, null).setUV(0f, 1f),
			data.vertTmp2.set(corner10, normal, null, null).setUV(1f, 1f),
			data.vertTmp3.set(corner11, normal, null, null).setUV(1f, 0f),
			data.vertTmp4.set(corner01, normal, null, null).setUV(0f, 0f), divisionsU, divisionsV);
	}
	...
	public static void build (MeshPartBuilder builder, float x00, float y00, float z00, float x10, float y10, float z10, float x11,
		...
		int divisionsV) {
		final BaseShapeData data = tlData.get();
		build(builder, data.vertTmp1.set(null).setPos(x00, y00, z00).setNor(normalX, normalY, normalZ).setUV(0f, 1f),
			data.vertTmp2.set(null).setPos(x10, y10, z10).setNor(normalX, normalY, normalZ).setUV(1f, 1f),
			data.vertTmp3.set(null).setPos(x11, y11, z11).setNor(normalX, normalY, normalZ).setUV(1f, 0f),
			data.vertTmp4.set(null).setPos(x01, y01, z01).setNor(normalX, normalY, normalZ).setUV(0f, 0f), divisionsU, divisionsV);
	}
}
