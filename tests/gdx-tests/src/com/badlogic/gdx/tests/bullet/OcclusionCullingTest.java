...
public class OcclusionCullingTest extends BaseBulletTest {
	...
	private enum CullingPolicy {
		 ...
		 * the Bullet broadphase bounding volume tree. Theoretically, this is more precise than the simple culling mode, since it
		 * checks axis aligned bounding boxes instead of bounding box radii. Therefore it will cull a few more entities. */
		KDOP;
	...
	public void create () {
		...
		// Add ground. Ignored in occlusion testing, since the flags are not set.
	...
	protected void renderWorld () {
		...
		if (cullingPolicy == CullingPolicy.NONE) {
			...
			// Brute force check all entities if they are inside inside frustum, or is ignored for culling purposes.
