...
public abstract class OcclusionCuller implements Disposable {
	...
	protected class Collider extends ICollide {
		...
		public boolean AllLeaves (btDbvtNode node) {
			if (node.isleaf()) {
				onObjectVisible(getCollisionObject(node));
			} else {
				long parentPointer = node.getCPointer();
				btDbvtNode child;
				if ((child = btDbvtNode.internalTemp(parentPointer, false).getChild(0)).getCPointer() != 0) AllLeaves(child);
				if ((child = btDbvtNode.internalTemp(parentPointer, false).getChild(1)).getCPointer() != 0) AllLeaves(child);
			}
			return true;
		}
		...
		public void Process (btDbvtNode leaf, float depth) {
			btCollisionObject object = getCollisionObject(leaf);
	...
	private static btCollisionObject getCollisionObject (btDbvtNode leaf) {
		btBroadphaseProxy proxy = btBroadphaseProxy.internalTemp(leaf.getData(), false);
		long clientPtr = proxy.getClientObject();
		if (clientPtr == 0) throw new NullPointerException("Failed to find collision object instance in node " + leaf);
		return btCollisionObject.getInstance(clientPtr);
	}
	 ...
	 * of shapes will be ignored. However, box occluders can block vision of any type of shape.
	 *
	...
	public void performKDOPCulling (btDbvtBroadphase broadphase, Camera camera) {
		...
		// Find objects inside the camera frustum
	 ...
	 *
	 * @param broadphase The dynamics world broadphase
	 * @param oclBuffer The occlusion buffer in which to draw depth during culling
	 * @param camera Camera for which to perform occlusion culling, will be used when querying and drawing the occlusion buffer */
	public void performOcclusionCulling (btDbvtBroadphase broadphase, OcclusionBuffer oclBuffer, Camera camera) {
		...
		setFrustumPlanes(camera.frustum);
		// Sort, query, render, collect visible objects in frustum
