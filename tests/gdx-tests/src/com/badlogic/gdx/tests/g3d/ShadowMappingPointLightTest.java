...
public class ShadowMappingPointLightTest extends GdxTest {
	...
	public void create () {
		...
		shadowLight = new PointShadowLight(1024, 1024, 30f, 30f, 1f, 100f);
		...
		modelBuilder.begin();
		mpb = modelBuilder.part("parts", GL20.GL_TRIANGLES, Usage.Position | Usage.Normal | Usage.ColorUnpacked, new Material(
	...
	public void render () {
		...
		float time = (TimeUtils.millis() % 10000) * 360f / 1000f / 10f;
		shadowLight.position.set(6 + 2 * MathUtils.cosDeg(time), 9 + 4 * MathUtils.cosDeg(2 * time),
			3 + 5 * MathUtils.cosDeg(3 * time));
		instance2.transform.setToTranslation(shadowLight.position);
