...
public class CursorTest extends GdxTest {
	...
	private Cursor cursorAndroid1;
	private Cursor cursorAndroid2;
	private Cursor cursorAndroid3;
	...
	public void create () {
		...
		if (!isAndroid) {
			...
			cursorAndroid1 = Gdx.graphics.newAndroidCursor("cursor", .5f, .5f);
			cursorAndroid2 = Gdx.graphics.newAndroidCursor("cursor2", .5f, .5f);
			cursorAndroid3 = Gdx.graphics.newAndroidCursor("cursor3", .5f, .5f);
		}
	}
	...
	public void render () {
		...
		if (!isAndroid) {
			...
			if (Gdx.input.isTouched()) {
				Gdx.graphics.setAndroidCursor(cursorAndroid3);
			} else {
				cursorActive = !cursorActive;
				if (cursorActive) {
					Gdx.graphics.setAndroidCursor(cursorAndroid2);
				} else {
					Gdx.graphics.setAndroidCursor(cursorAndroid1);
				}
			}
		}
	}
