...
public class ViewportTest6 extends GdxTest {
...
// viewports.add(new DoubleRatioViewport(300, 200, 600, 400));
	...
	public void render () {
		...
		batch.draw(border, 0, 0, viewport.getViewportX() / 2, Gdx.graphics.getHeight() / 2, viewport.getViewportX(),
			Gdx.graphics.getHeight(), 1, 1, 0, 0, 0, border.getWidth(), border.getHeight(), true, false);

		// draw the right bar green
		batch.draw(border, Gdx.graphics.getWidth() - viewport.getViewportX(), 0, viewport.getViewportX() / 2,
			Gdx.graphics.getHeight() / 2, viewport.getViewportX(), Gdx.graphics.getHeight(), 1, 1, 0, 0, 0, border.getWidth(),
			border.getHeight(), false, false);

		// draw the top bar green
		batch.draw(border, 0, Gdx.graphics.getHeight() - viewport.getViewportY(), Gdx.graphics.getWidth() / 2,
			viewport.getViewportY() / 2, Gdx.graphics.getWidth(), viewport.getViewportY(), 1, 1, 0, 0, 0, border.getWidth(),
			border.getHeight(), false, false);

		// draw the bottom bar green
		batch.draw(border, 0, 0, Gdx.graphics.getWidth() / 2, viewport.getViewportY() / 2, Gdx.graphics.getWidth(),
			viewport.getViewportY(), 1, 1, 0, 0, 0, border.getWidth(), border.getHeight(), false, true);
