...
public class TiledDrawableTest extends GdxTest {
	...
	public void create () {
		...
		textureRegion = new TextureRegion(new Texture(Gdx.files.internal("data/tiles_drawable_test.png")));
	...
	public void render () {
		...
		final float clusterWidth = Math.max(13, (inputX - (3 * spacingX)) / 3f);
		final float clusterHeight = Math.max(13, (inputY - (3 * spacingY)) / 3f);

		final float leftX = spacingX;
		final float centerX = (2 * spacingX) + clusterWidth;
		final float rightX = (3 * spacingX) + (2 * clusterWidth);
		final float topY = (3 * spacingY) + (2 * clusterHeight);
		final float centerY = (2 * spacingY) + clusterHeight;
		final float bottomY = spacingY;
