...
/** A LibGDX application that demonstrates how to interact with the window system using the LWJGL3 backend. It showcases lifecycle
 * events, handling keyboard input, window management (fullscreen, windowed, visibility, maximize, etc.), and basic rendering with
 * sprite batch and fonts.
 * <p>
 * Users can test various window functionalities using specific key presses, such as: - 'f' for fullscreen mode. - 'w' for random
 * window size. - 'e' to throw an exception. - 'c' to toggle cursor capture. - 'v' to hide the window, 's' to show it. - 'q' to
 * close the window. - 'i' to iconify the window. - 'm' to maximize the window. - 'r' to restore the window. - 'u' to open a URL
 * in the browser. */
public class Lwjgl3DebugStarter {
	/** Main method to start the application. This sets up the LibGDX application configuration, initializes the test, and starts
	 * the rendering loop.
	 * 
	...
	public static void main (String[] argv) throws NoSuchFieldException, SecurityException, ClassNotFoundException {
		GdxTest test = new GdxTest() {
			...
			/** Creates the test environment, including setting up the input listener, texture, font, and lifecycle listeners. */
			@Override
			public void create () {
...
// }
			...
			/** Renders the frame by clearing the screen, drawing FPS data, handling window resizing, and displaying the texture. */
			@Override
			public void render () {
