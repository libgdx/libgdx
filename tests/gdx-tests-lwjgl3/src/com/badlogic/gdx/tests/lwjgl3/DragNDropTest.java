 ...
 * 
 * This test uses libGDX to manage the UI and event handling. When files are dropped,
 * their paths are shown on the screen as labels.
 * @author mbrlabs */
public class DragNDropTest extends GdxTest {
	...
	private Table root;
 /**
     * Initializes the test by setting up the stage, skin, and input processor.
     * Creates a root Table that will hold the labels for dropped files.
     */
	@Override
	public void create () {
     ...
     * It is not used in this test, but it must be implemented to fulfill the
...
/**
 ...
 /**
     * This method is called when the application is disposed of (e.g., when the window is closed).
     * It is not used in this test, but it must be implemented to fulfill the
     * ApplicationListener interface.
     */
	@Override
...
/**
     * Adds file paths to the UI as labels within the root table.
     * Each file path dropped into the window is displayed as a label in the window.
     * 
...
/**
     * The entry point of the application. Configures the window and creates the test application.
     * Sets up the GLFW window and registers a file drop callback that adds dropped files to the UI.
     * 
